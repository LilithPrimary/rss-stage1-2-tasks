...
function restoreColors(options: IFilterOptions) {
  ...
  colorBtns.forEach(el => {
    if (options.color.includes(<string>el.firstElementChild?.id)) el.classList.add("color-filter__color-wrapper--checked");
  });
}
...
function restoreProducers(options: IFilterOptions) {
  ...
  producedBtns.forEach(el => {
    if (options.produced.includes(<string>el.firstElementChild?.id)) el.classList.add("produced-filter__wrapper--checked");
  });
}
...
function restoreOther(options: IFilterOptions) {
  ...
  otherBtns.forEach(el => {
    if (el.id === "isLike" && options.isLike[0]) el.classList.add("other-filter__wrapper--checked");
    if (el.id === "vegan" && options.vegan[0]) el.classList.add("other-filter__wrapper--checked");
    if (options.type.includes(el.id)) el.classList.add("other-filter__wrapper--checked");
  });
}
